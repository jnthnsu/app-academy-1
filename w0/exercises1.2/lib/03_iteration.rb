...
def substrings(string)
	all = (0..string.length - 1).inject([]) do |list, i1|
		list + (i1..string.length - 1).inject([]) do |sublist, i2|
			sublist << string[i1..i2]
		end
	end
	all.uniq
end
...
class Array
  def my_map(&prc)
	  new = []
	  self.my_each{|x| new << prc.call(x)}
	  new
  end

  def my_select(&prc)
	  new = []
	  self.my_each{|x| new << x if prc.call(x)}
	  new
  end

  def my_inject(&blk)
	  new = self[0]
	  self[1..-1].my_each{|x| new = blk.call(new, x)}
	  new
  end
end
...
def concatenate(strings)
	strings.inject{|new, string| new + string}
end
