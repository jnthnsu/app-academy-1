...
class Array
  def my_uniq
	  unique = []
	  self.each{|x| unique << x unless unique.include?(x)}
	  unique
  end
end
...
class Array
  def two_sum
	  ...
	  self.each_with_index do |n1, i1|
		  self[i1 + 1..-1].each_with_index do |n2, i2|
			  pairs << [i1, i1 + i2 + 1] if n1 == -n2
		  end
	  end
	  pairs
  end
...
class Array
  def median
	  ...
	  sorted = self.sort
	  return sorted[self.length/2] if self.length.odd?
