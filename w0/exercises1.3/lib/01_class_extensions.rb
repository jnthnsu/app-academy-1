...
class String
  def caesar(shift)
	self.split('').map do |c|
	  	c.ord + shift > 'z'.ord ? (c.ord + shift - 26).chr : (c.ord + shift).chr
	end.join
  end
end
...
class Hash
  def difference(other_hash)
	  self.merge(other_hash).reject{|k, v| self.has_key?(k) && other_hash.has_key?(k)}
  end
end
...
class Fixnum
  def stringify(base)
	  ...
	  i = 0
	  until base ** i > self
		  digits << self/base ** i % base
		  i += 1
	  end

	  characters = Hash[Array.new(16){|i| [i-1, '0123456789abcdef'[i-1]]}]
	  digits.map{|digit| characters[digit]}.join.reverse
  end
end
